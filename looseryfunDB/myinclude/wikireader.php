...
function getScenarioURLs(){
	$urls=[
		'https://www.dopr.net/toramonline-wiki/scenario1_01',
		'https://www.dopr.net/toramonline-wiki/scenario1_02',
		'https://www.dopr.net/toramonline-wiki/scenario1_03',
		'https://www.dopr.net/toramonline-wiki/scenario1_04',
...
function generateScenarioExpData($url){
	static $chapterQuery = '//div[@id="main-content"]//h1';
	static $query = '//div[@id="main-content"]//h2';
	$result = array();
	$domDocument = new DOMDocument();
	if( !@$domDocument->loadHTMLFile($url) )return $result;
	...
	// 章の名前取得
	$chapterName ='';
	$nodes = $xpath->query($chapterQuery);
	foreach($nodes as $node) {
		$chapterName = $node->textContent;
	}
	$pos = strpos($chapterName,'】');
	if($pos>0){
		$chapterName = substr($chapterName,$pos+strlen('】'));
		$chapterName = preg_replace('/^[ 　]+/u', '', $chapterName);
		$chapterName = preg_replace('/[ 　]+$/u', '', $chapterName);
		$chapterName = trim($chapterName);
	}
	$result['name'] = $chapterName;
	...
	foreach($nodes as $node) {
		...
		if( preg_match('/EXP.?\d+/u', $innnerText, $expString) >0){
			$exp = (int)preg_replace('/[^\-\d]*(\-?\d*).*/','$1',$expString[0]);
			$expData[] = ['name'=>$title,'exp'=>$exp];
		}
	}
	$result['sub'] = $expData;
	return $result;
}
function generateAllScenarioExpData(){
	$allUrl = getScenarioURLs();
	$result = array();
	foreach($allUrl as $url){
		$result[] = generateScenarioExpData($url);
	}
	return $result;
}
function getAllScenarioExpData(){
	return apcu_entry('AllScenarioExpData', 'generateAllScenarioExpData', URL_CACHETIME);
}
?>
